...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        m_sprintParticleSpawned = DTKParticleManager.m_instance.SpawnParticle(m_sprintParticle, transform.position, transform.eulerAngles, 1.0f, false, transform);
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.LeftShift))
        {
            ...
            m_sprintParticleSpawned.Play();
        }

        if (Input.GetKeyUp(KeyCode.LeftShift))
        {
            m_currSpeed = m_walkSpeed;
            m_sprintParticleSpawned.Stop();
        }

        //GroundCheck
        //////////////////////////////////////////
        if (Physics.Raycast(m_rayOrigin.position, Vector3.down, m_rayDistance, m_floorLayer))
        {
            m_grounded = true;
        }
        ...
        if (m_grounded)
        {
            ...
            {
                AudioManager.m_instance.PlayerClip(DTK_AUDIOSOURCE.ksfx, m_jumpSound, 1.0f);
                DTKParticleManager.m_instance.SpawnParticle(m_dustParticle, transform.position, transform.eulerAngles, 1.0f);
                isJumping = true;
